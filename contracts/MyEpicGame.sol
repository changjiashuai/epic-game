...
contract MyEpicGame is ERC721 {
    ...
    function attackBoss() public {
        ...
        if (bigBoss.hp < player.attackDamage) {
            ...
            if (randomInt(30) > 5) {// by passing 10 as the mod, we elect to only grab the last digit (0-9) of the hash!
        ...
        if (player.hp < bigBoss.attackDamage) {
            player.hp = 0;
            console.log("The %s is dead!", player.name);
        } else {
            if (randomInt(10) > 5) {
                player.hp = player.hp - bigBoss.attackDamage;
                console.log("boss attacked %s. New %s hp: %s", player.name, player.name, player.hp);
            }
        }
    ...
    function randomInt(uint _modulus) internal returns (uint) {
        ...
        return uint(keccak256(abi.encodePacked(block.timestamp,
            ...
            randNonce))) % _modulus;
        // modulo using the _modulus argument
    }
